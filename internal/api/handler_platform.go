...
import (
	...
	"github.com/jiujiu532/Resin/internal/service"
)
...
// HandleListBlockedNodes returns a handler for GET /api/v1/platforms/{id}/blocked-nodes.
func HandleListBlockedNodes(cp *service.ControlPlaneService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, ok := requireUUIDPathParam(w, r, "id", "platform_id")
		if !ok {
			return
		}
		items, err := cp.ListBlockedNodes(id)
		if err != nil {
			writeServiceError(w, err)
			return
		}
...
// HandleBlockNode returns a handler for POST /api/v1/platforms/{id}/blocked-nodes.
func HandleBlockNode(cp *service.ControlPlaneService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, ok := requireUUIDPathParam(w, r, "id", "platform_id")
		if !ok {
			return
		...
		if req.NodeHash == "" {
			writeInvalidArgument(w, "node_hash is required")
			return
		}
		p, err := cp.BlockNode(id, req.NodeHash)
		if err != nil {
			writeServiceError(w, err)
			return
		}
...
// HandleUnblockNode returns a handler for DELETE /api/v1/platforms/{id}/blocked-nodes/{hash}.
func HandleUnblockNode(cp *service.ControlPlaneService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, ok := requireUUIDPathParam(w, r, "id", "platform_id")
		if !ok {
			return
