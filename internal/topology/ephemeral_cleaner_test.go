...
//
// Timeline:
//  1. Node is circuit-broken with stale CircuitOpenSince â†?enters evictSet.
//  2. betweenScans hook fires: clears CircuitOpenSince (simulating recovery).
//  3. Second check re-reads CircuitOpenSince=0 â†?node is NOT confirmed.
...
func TestEphemeralCleaner_TOCTOU_RecoveryBetweenScans(t *testing.T) {
	...
	if !hookCalled {
		t.Fatal("betweenScans hook was not called â€?node may not have been a candidate")
	}
