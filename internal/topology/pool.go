// Package topology coordinates the subscription â†?node pool â†?platform view pipeline.
...
type GlobalNodePool struct {
	...
	// Subscription lookup â€?injected by SubscriptionManager.
	subLookup func(subID string) *subscription.Subscription

	// GeoIP lookup â€?injected at construction.
...
func (p *GlobalNodePool) RecordLatency(hash node.Hash, rawTarget string, latency *time.Duration) {
	...
	// now satisfy the HasLatency filter â€?notify platforms.
	if wasEmpty {
		p.notifyAllPlatformsDirty(hash)
	}
