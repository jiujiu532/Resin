...
import (
	...
	"github.com/jiujiu532/Resin/internal/state"
)
...
// BlockedNodeItem is the API response for a single blocked node entry.
type BlockedNodeItem struct {
	NodeHash   string `json:"node_hash"`
	DisplayTag string `json:"display_tag,omitempty"`
}

// ListBlockedNodes returns the blocked node list for a platform.
func (s *ControlPlaneService) ListBlockedNodes(platformID string) ([]BlockedNodeItem, error) {
	mp, err := s.getPlatformModel(platformID)
	if err != nil {
		return nil, err
	}
	result := make([]BlockedNodeItem, 0, len(mp.BlockedNodeHashes))
	for _, hexStr := range mp.BlockedNodeHashes {
		item := BlockedNodeItem{NodeHash: hexStr}
		if h, parseErr := node.ParseHex(hexStr); parseErr == nil {
			if s.Pool != nil {
				item.DisplayTag = s.Pool.ResolveNodeDisplayTag(h)
			}
		}
...
// BlockNode adds a node to the platform's blocklist and rebuilds the routable view.
func (s *ControlPlaneService) BlockNode(platformID, nodeHash string) (*PlatformResponse, error) {
	...
	mp.BlockedNodeHashes = append(mp.BlockedNodeHashes, nodeHash)
...
// UnblockNode removes a node from the platform's blocklist and rebuilds the routable view.
func (s *ControlPlaneService) UnblockNode(platformID, nodeHash string) (*PlatformResponse, error) {
	if _, err := node.ParseHex(nodeHash); err != nil {
		return nil, invalidArg("node_hash: invalid hex format")
	}
	mp, err := s.getPlatformModel(platformID)
	...
	if !found {
		return nil, notFound("node not in blocked list")
	}
