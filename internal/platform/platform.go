...
func NewPlatform(id, name string, regexFilters []*regexp.Regexp, regionFilters []string) *Platform {
	return &Platform{
		...
		blockedNodes:  make(map[node.Hash]struct{}),
		view:          NewRoutableView(),
	}
}

// SetBlockedNodes replaces the platform's blocked node set.
// hashes is a slice of node.Hash values to block.
func (p *Platform) SetBlockedNodes(hashes []node.Hash) {
...
// BlockedNodeHexes returns the blocked node hashes as hex strings.
func (p *Platform) BlockedNodeHexes() []string {
	result := make([]string, 0, len(p.blockedNodes))
	for h := range p.blockedNodes {
		result = append(result, h.Hex())
	}
	return result
}

// IsNodeBlocked reports whether the given node hash is in the platform's block list.
func (p *Platform) IsNodeBlocked(h node.Hash) bool {
