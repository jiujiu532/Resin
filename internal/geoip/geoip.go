...
func (s *Service) Start() error {
	...
	if err == nil {
		...
		// No local database at all â€?download immediately in background.
...
// cron schedule interval. Uses 2Ã— the gap between two consecutive cron
...
func (s *Service) isStale(modTime time.Time) bool {
	...
	if entry.ID == 0 || entry.Schedule == nil {
		// Cron not configured â€?fall back to conservative default.
	...
	// Stale if mtime is older than 2Ã— the interval.
